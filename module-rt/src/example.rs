...
use remote_trait_object::{Context as RtoContext, Dispatch, HandleToExchange, Service};
use std::sync::Arc;
...
impl UserModule for ModuleA {
    ...
    fn prepare_service_to_export(&mut self, ctor_name: &str, ctor_arg: &[u8]) -> Arc<dyn Dispatch> {
        ...
        Arc::new(HelloBoxDispatcher::new(Box::new(SimpleHello {
            value,
...
mod tests {
    ...
    fn create_module(
        ...
        n: usize,
        init: &[u8],
    ) -> (ExecutorContext<Intra, PlainThread>, RtoContext, Box<dyn crate::coordinator_interface::FoundryModule>) {
